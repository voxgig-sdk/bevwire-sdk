...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        BevwireSDK $client,
        BevwireUtility $utility
    ): BevwireContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new BevwireContext($ctxmap, null);
    ...
    private static function fixctx(BevwireContext $ctx, BevwireSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?BevwireError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BevwireError($code, $msg);
    }
    ...
    {
        $client = BevwireSDK::test(null, null);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new BevwireBaseFeature();
    ...
    {
        $hook_client = BevwireSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends BevwireBaseFeature {
            public $hook_fn = null;
            public function TestHook(BevwireContext $ctx): void
            {
    ...
    {
        $init_client = BevwireSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends BevwireBaseFeature {
            public $init_fn = null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new BevwireSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = BevwireSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = BevwireSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof BevwireError) ? $result->err->msg : (string)$result->err;
