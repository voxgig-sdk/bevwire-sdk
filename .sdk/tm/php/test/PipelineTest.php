...
{
    private static function utility(): BevwireUtility
    {
        return new BevwireUtility();
    }
    ...
    private static function ctx(array $over = []): BevwireContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BevwireContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BevwireOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BevwireError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BevwireOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BevwireMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BevwireOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BevwireMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BevwireOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BevwireMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BevwireSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BevwireMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BevwireResponse([]),
            'result' => new BevwireResult([]),
        ]);
        [, $err] = BevwireMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BevwireSpec([]),
            'response' => null,
            'result' => new BevwireResult([]),
        ]);
        [, $err] = BevwireMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BevwireSpec(['step' => 's']),
            'response' => new BevwireResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BevwireResult([]),
        ]);
        [, $err] = BevwireMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BevwireSpec(['step' => 's']),
            'response' => new BevwireResponse(self::resp(200, ['v' => 1])),
            'result' => new BevwireResult([]),
        ]);
        [, $err] = BevwireMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BevwireSpec(['step' => 's']),
            'response' => new BevwireResponse(self::resp(200, ['v' => 2])),
            'result' => new BevwireResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BevwireMakeResponse::call($ctx);
    ...
    {
        $preset = new BevwireResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BevwireSpec([]),
            'response' => new BevwireResponse([]),
            'result' => new BevwireResult([]),
        ...
        [$got, $err] = BevwireMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BevwireResult([])]);
        [, $err] = BevwireMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BevwireSpec([]), 'result' => null]);
        [, $err] = BevwireMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BevwireSpec(['step' => 's']),
            'result' => new BevwireResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BevwireOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BevwireMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BevwireSpec(['step' => 's']),
            'result' => new BevwireResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BevwireOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BevwireMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BevwireResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BevwireSpec([]),
            'result' => new BevwireResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BevwireMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BevwireMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BevwireError('boom', 'boom');
        $utility->fetcher = function (BevwireContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BevwireSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BevwireMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BevwireContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BevwireMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BevwireResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BevwireContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BevwireSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BevwireMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BevwireResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BevwireSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BevwireMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BevwireMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BevwireMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BevwireResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BevwireDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BevwireResult(['ok' => false])]);
        $this->expectException(BevwireError::class);
        BevwireDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BevwireResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BevwireMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BevwireMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BevwireBaseFeature();
        $b = new BevwireBaseFeature();
        BevwireFeatureAdd::call($ctx, $a);
        BevwireFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BevwireBaseFeature
    {
        $f = new BevwireBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BevwireFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BevwireContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BevwireSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BevwirePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BevwireResponse(['headers' => null]),
            'result' => new BevwireResult([]),
        ]);
        BevwireResultHeaders::call($ctx);
