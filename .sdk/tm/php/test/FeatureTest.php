...
class FtCtrl extends BevwireControl
{
...
class FtClient
{
    ...
    private BevwireUtility $utility;

    public function __construct(BevwireUtility $utility, array $options)
    {
    ...
    public function get_utility(): BevwireUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BevwireContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BevwireUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BevwireContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BevwireConfig::make_feature($name);
    ...
    private static function buildUrl(BevwireSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BevwireError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BevwireResponse($fetched);
        }
    ...
    private function fail(BevwireContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BevwireContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
