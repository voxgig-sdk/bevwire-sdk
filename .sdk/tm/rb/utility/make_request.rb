...
module BevwireUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = BevwireResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BevwireResponse.new(fetched)
    else
