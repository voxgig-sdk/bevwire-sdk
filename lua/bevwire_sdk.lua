...
BevwireSDK._make_feature = _make_feature


function BevwireSDK.new(options)
  local self = setmetatable({}, BevwireSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function BevwireSDK:options_map()
...
function BevwireSDK:get_utility()
  return Utility.copy(self._utility)
end


function BevwireSDK:get_root_ctx()
  return self._rootctx
end


function BevwireSDK:prepare(fetchargs)
...
function BevwireSDK:direct(fetchargs)
...
-- by passing it.
function BevwireSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function BevwireSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "BevwireSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function BevwireSDK:Article(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function BevwireSDK:Company(data)
...
function BevwireSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = BevwireSDK.new(sdkopts)
  sdk.mode = "test"
