...
const TEST_SEED = {"entity":{"article":{"test01":{"id":"test01"}},"company":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+BevwireSDK\b/.test(code) ||
    /\bBevwireSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BevwireSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BevwireSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
