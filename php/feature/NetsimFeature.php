...
class BevwireNetsimFeature extends BevwireBaseFeature
{
    ...
    private function _simulate(BevwireContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(BevwireContext $ctx, array $applied): void
    {
        $client = $this->client;
