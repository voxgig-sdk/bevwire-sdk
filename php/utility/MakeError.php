...
class BevwireMakeError
{
    public static function call(?BevwireContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BevwireContext([], null);
        }
        $op = $ctx->op ?? new BevwireOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BevwireResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BevwireError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BevwireError('', $msg, $ctx);
        ...
        if ($err instanceof BevwireError) {
