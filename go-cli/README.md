...
boru-driven command-line client **and** interactive REPL for the Bevwire
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bevwire-cli)
make build
...
./bevwire-cli load 1 article            # {id:1} shorthand
./bevwire-cli load '{id:1}' article       # explicit match map

# 5. Override the API base URL for a single call
BEVWIRE_BASE=https://api.example.com ./bevwire-cli load 1 article

# 6. No arguments -> interactive REPL
./bevwire-cli
bevwire> load 1 article
bevwire> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bevwire-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bevwire-cli load 1 article
   ```
...
```sh
./bevwire-cli load 1 article          # scalar shorthand for {id:1}
./bevwire-cli load '{id:1}' article     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bevwire>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bevwire-cli`. |
...
(`./bevwire-cli <expr>`) and interactively (the REPL), and expressions compose the
