// Command Bevwire-cli is a CLI + REPL for the Bevwire SDK,
...
const prompt = "bevwire"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "article company"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BEVWIRE_APIKEY (api key), BEVWIRE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BEVWIRE_APIKEY carries the API key and
	// BEVWIRE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BEVWIRE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BEVWIRE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBevwireSDK(opts)
