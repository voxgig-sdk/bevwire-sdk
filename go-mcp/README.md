...
[MCP](https://modelcontextprotocol.io) server exposing the Bevwire SDK as
two agent tools — `bevwire_list` and `bevwire_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bevwire-mcp)
make build

# 2. Provide credentials via the environment
export BEVWIRE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bevwire \
  -- /absolute/path/to/bevwire-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bevwire-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bevwire_list: first page of records
{ "entity": "article" }
{ "entity": "article", "query": { } }

// bevwire_load: one record by id
{ "entity": "article", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bevwire-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bevwire \
     -- "$PWD"/dist/*/bevwire-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bevwire_list` and `bevwire_load` tools now appear
   in new sessions. Ask the agent to *"list article using bevwire"*
   and it calls `bevwire_list` with `{"entity":"article"}`.
...
```sh
export BEVWIRE_APIKEY=sk_live_xxx            # API key
export BEVWIRE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `bevwire_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bevwire_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BEVWIRE_APIKEY` | API key sent with every request. |
| `BEVWIRE_BASE` | Optional override of the API base URL. |
...
```sh
./bevwire-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bevwire_load","arguments":{"entity":"article","query":{"id":1}}}}'
```
