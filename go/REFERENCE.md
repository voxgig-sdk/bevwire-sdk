# Bevwire Golang SDK Reference

Complete API reference for the Bevwire Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *BevwireSDK`
...
#### `Article(data map[string]any) BevwireEntity`

Create a new `Article` entity instance. Pass `nil` for no initial data.

#### `Company(data map[string]any) BevwireEntity`
...
```go
result, err := client.Article(nil).Load(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
