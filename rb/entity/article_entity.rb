...
class ArticleEntity
  ...
  def data_set(args)
    if args
      @_data = BevwireHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BevwireHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Article, Hash] the loaded Article; raises BevwireError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BevwireHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
